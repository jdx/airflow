...
- [ ] Generate a file-filter function in `hk.pkl` (emitted by the migrator) to emulate `types`/`types_or` per-hook without adding that concept to hk core
...
### File filtering with existing hk mechanics (generated by migrate)

hk already filters per-step files using `dir` → `glob` → `exclude` (see hk `Step.filter_files`), where `glob` and `exclude` are glob patterns. We will generate globs that emulate pre-commit `types`/`types_or` and assign them to each step’s `glob` so hk’s native filtering does the work.

- [ ] Migrator emits a local function that returns a `List(String)` of glob patterns, e.g. `globsForKinds(kinds)`
- [ ] Implement canonical kind→glob mappings:
  - yaml → `.yml,.yaml`
  - python → `.py`
  - pyi → `.pyi`
  - json → `.json`
  ...
  - rst → `.rst`
  - shell → `.sh,.bash`
  - text → common text-like extensions (or fallback: exclude known binaries)
  - file → any regular file (exclude dirs)
- [ ] Each migrated step sets `glob = globsForKinds([...])` (optionally concatenated with any specific path globs derived from `files:`)
- [ ] Preserve per-hook `files:` and `exclude:` semantics: translate simple `files:` regexes to additional globs; for non-trivial regex cases, fall back to a `check_list_files` helper command to further narrow files
- [ ] No new hk feature required; this is pure config generation in `hk.pkl` and uses hk’s existing `glob`/`exclude` filtering

Example (conceptual Pkl):

```pkl
local kindsToGlobs = new Mapping<String, List<String>> {
    ["yaml"] = List("**/*.yml", "**/*.yaml"),
    ["python"] = List("**/*.py"),
    ["pyi"] = List("**/*.pyi"),
}

fun globsForKinds(kinds: List<String>): List<String> = kinds
    .flatMap(k -> kindsToGlobs.getOrElse(k, List()))

// In a step converted from types_or: [python, pyi]
glob = globsForKinds(List("python", "pyi"))
```
...
- when working on this, update both hk and airflow repos. Check off work items in plan.md as you complete things.
- we'll want to build hk and use the generator to rebuild airflow's hk.pkl file.
